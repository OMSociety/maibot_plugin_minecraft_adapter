...
[![Version](https://img.shields.io/badge/version-1.3.1-blue.svg)](https://github.com/OMSociety/maibot_plugin_minecraft_adapter)
...
- `/mc mybind` —— 查看自己的绑定状态（Java 版与基岩版各一条，同时显示）
- `/mc unbind` —— 解除**全部**绑定并尝试移出白名单
...
| `/mc bind <游戏ID>` | 绑定 Java 版游戏 ID 并加入白名单 | 公开 |
| `/mc geyserbind <基岩版ID>` | 绑定基岩版 ID（Floodgate） | 公开 |
| `/mc mybind` | 查看自己的绑定（Java 版与基岩版各一条） | 公开 |
