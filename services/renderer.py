...
class InfoRenderer:
    ...
    def _font(self, size: int) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
        ...
        # 2. 尝试插件数据根目录下的 font.ttf (用户自定义)
        try:
            if self._data_dir:
                custom_font = Path(self._data_dir) / "font.ttf"
    ...
    @staticmethod
    def _safe_percent(value: float, lo: int = 0, hi: int = 100) -> int:
        try:
